...
This Netpython for the python environment is a advanced networking utility for reading 
...
network purposes, also can be a "back-end" tool that can run programs and scripts remotely, 
easily upon the connection. This netpython is an idea from netcat which is a feature-rich 
network debugging and exploration tool, have inspired the author to create this tool. The 
author belives that this tool can be a netcat alternative solution for minimize cyber attacks 
(replace netcat with netpython can stop hackers from create a reverse or bind shell, except 
hackers also known that the server is using netpython as netcat) and for who are working with 
python enviroment or who are using Windows which doesn't have netcat as the default. Special, 
...
using python 2, use "np2.py". I am working on "np2" for I/O multi-threading.


Some netpython features:
 ...
 - Executing programs upon the connection
 - Server chatting
 - Network debugging
 - Traffic dumping
Netpython can also be used as a backdoor.
...
netcat for Windows, but his Anti-virus has blocked his netcat. He was so sad and just 
...
...). Author has tried to doesn't use any "required download" modules. Anyway, the special in this 
tool is something the thing I call "Virtual-console". That when you create a bind or a reverse shell, 
it looks like you have created a shell, but the true it just check the output of your command and do
...
netcat, he will do it in the future with a bigger project. 
